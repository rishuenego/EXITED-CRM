...
router.post('/', authenticateSession, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (!employee_id || !exit_type || !exit_date) {
      return res.status(400).json({ error: 'Employee, exit type, and exit date are required' })
    }
    ...
    res.status(201).json({ id: exitRecordId, message: 'Exit record created successfully' })
  } catch (error) {
...
router.put('/:id', authenticateSession, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await connection.execute('DELETE FROM exit_credentials WHERE exit_record_id = ?', [req.params.id])
...
router.delete('/:id', authenticateSession, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [rows] = await pool.execute('SELECT employee_id FROM exitrecords_table WHERE id = ?', [req.params.id])
    const exits = rows as any[]

    if (exits.length > 0) {
      // Optionally reactivate employee
      await pool.execute('UPDATE employees_table SET status = ? WHERE id = ?', ['active', exits[0].employee_id])
    }

    await pool.execute('DELETE FROM exitrecords_table WHERE id = ?', [req.params.id])
